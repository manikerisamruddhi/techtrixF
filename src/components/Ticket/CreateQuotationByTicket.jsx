...
const CreateQuotationModal = ({ ticketId, visible, onCancel }) => {
  ...
  const [isExistingCustomer, setIsExistingCustomer] = useState(false); // New state for customer type

  useEffect(() => {
    if (ticketId) {
      setIsExistingCustomer(true); // Set existing customer as default when ticketId is passed
    }
  }, [ticketId]);
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" name="quotationForm">
        ...
        {/* Add existing/new customer section */}
        <Form.Item
          label="Customer Type"
        >
          <Input value={isExistingCustomer ? "Existing Customer" : "New Customer"} disabled />
        </Form.Item>
